...
module.exports = {
  ...
  update: async (req, res, next) => {
    try {
      res.status(200)
        .json(await models.Articulo.update(
  ...
  activate: async (req, res, next) => {
    try {
      res.status(200)
        .json(await models.Articulo.update({ estado: 1 }, { where: { id: req.body.id } }));
    } catch (e) {
      res.status(500).send({ message: 'Ocurrió un error' });
      next(e);
    }
  ...
  deactivate: async (req, res, next) => {
    try {
      res.status(200)
        .json(await models.Articulo.update({ estado: 0 }, { where: { id: req.body.id } }));
    } catch (e) {
      res.status(500).send({ message: 'Ocurrió un error' });
      next(e);
    }
